# Problem Statement Canvas: auxd (Social Album Platform)
...
3. **Differentiation thinness vs. AOTY / Letterboxd-for-music attempts.** Several small competitors exist. "Modern UX + social-first" is a positioning, not a moat. Phase 1 must identify *why* prior attempts stalled and what concrete capability separates auxd.
...
- **H4 (Integration feasibility):** Spotify Web API supports the listening-history, playback-context, and library-sync endpoints auxd needs, under stable rate limits and ToS-permitted use. Apple Music likewise.
