...
<!-- CR-002: differentiator rationale softened. ML is out-of-MVP for capacity/data reasons, not a values rejection. Social-graph is the rec primitive AT MVP; ML earns its keep once we have the data + scale to feed it. -->
| Differentiator | Social-graph recs at MVP; ML when it earns its keep | At MVP scale the data + the graph aren't dense enough for ML-based recs to outperform a transparent heuristic; the social-graph is the rec primitive that compounds while we build that data. ML (collaborative filtering, embeddings, ALS) is a v2 capacity question, not a values rejection. *(Pre-CR-001 rationale also cited streaming-API deprecation; CR-001 removed that bullet. Pre-CR-002 rationale framed this as 'algorithm distrust' — softened: the wedge is data-stage-aware, not anti-algorithm.)* | research/tech-stack.md |
...
<!-- CR-002: Q3 rationale softened. MVP behavior (heuristic + weights) UNCHANGED; the "algorithm distrust" framing was misleading — ML is deferred because MVP-scale data is too thin to feed it, not because the wedge rejects algorithms. -->
| 3 | Feed composition algorithm | **Reverse-chronological** from followed users, **weighted** toward (a) entries with a written review attached, (b) ★★★★★ and ★ extreme ratings, (c) entries by users the viewer interacts with most | Transparent, explainable, no ML at MVP scale (graph + data aren't dense enough for ML to outperform a clear heuristic yet); ML returns when scale + data earn its keep. *(Pre-CR-002 framed this as 'avoids algorithm distrust' — softened: see Differentiator row for the broader framing.)* | LOCKED (v1.5; rationale softened by CR-002) |
...
<!-- CR-002: NT-2 hero changed from single → three-hero carousel. -->
| NT-2 | Weekly digest includes hero entry? | **Yes — three-hero carousel: most-rated, most-reviewed, most-Aux'd in your follow graph this week.** Three editorial picks instead of one; stronger email-open hook. Three cheap aggregate queries (one per metric) — still cheap. *(v1.2: single hero. v1.5/CR-002: expanded to three-hero carousel after Phase-2 review pass surfaced that one hero under-uses the digest's prime real estate.)* |
...
<!-- CR-002: SS-3 reversed — invite landing now shows the "X just joined" line. Privacy gate via opt-in flag (default OFF for cohorts L-12..L-1; default ON post-launch). -->
| SS-3 | Invite link includes "X just joined" social-proof line? | **Yes — ON post-launch, gated by `User.visible_in_just_joined` opt-in flag.** Default ON for users who sign up at L 0 and later; default OFF for the closed-beta cohorts (L-12 through L-1) so the launch-wave narrative stays under founder control. Users can flip the flag in Settings → Privacy at any time. The "X just joined" line shows on `/i/{handle}` invite landing pages and on a `/joined` ticker that appears in onboarding step 1 (lightweight social proof, not personally targeted). *(v1.2: no at MVP. v1.5/CR-002: reversed after Phase-2 review pass — viral lift outweighs the launch-narrative concern once the closed-beta cohort gate is in place.)* |
...
<!-- CR-002: UJ-3 changed — feed loses inline expand; reviews live on a dedicated /review/:id route (Letterboxd parity). -->
| UJ-3 | Review expanded inline vs dedicated reading view? | **Dedicated `/review/:id` route only.** No inline expand in the feed. Feed shows rating + album thumbnail + first ~80 chars of review text as preview; tapping anywhere on the preview navigates to the route. Reading view surface: full hero (album cover, title, artist, viewer's rating, Aux badge if any, Like button, share). Letterboxd parity. The route is URL-shareable with OG meta and naturally hosts the v2 screenshot image-gen surface (UJ-4 v2). *(v1.2: inline expand. v1.5/CR-002: shifted to dedicated route only after Phase-2 review pass; predictable UX + clean setup for shareable review URLs.)* |
<!-- CR-002: UJ-4 unchanged at MVP; v2 roadmap adds screenshot image-gen on top of the new /review/:id route. -->
| UJ-4 | Share reviews via API cross-post (Twitter/X) or share-card only? | **Share-card only at MVP.** Cross-post APIs are deprecating across platforms; share-card with OG preview is portable and platform-risk-free. *(CR-002 roadmap note: v2 adds **screenshot image-gen** — server-side PNG rendering of the `/review/:id` hero — for richer share-sheet payloads. ~5–7 day add, only meaningful once the dedicated reading-view route exists, which is why it's queued for v2 rather than MVP.)* |
...
<!-- CR-002: row appended for the Phase-2 decision review pass. -->
| 2026-05-22 | **v1.6: Revision #5 (CR-002) — Phase 2 decision review pass** | Reason: founder re-walked all 22 main + 14 supporting (the 6 superseded by CR-001 + the SS-1 dupe of Q13 excluded) Phase 2 open questions; auto-resolved defaults had been applied without explicit confirmation in v1.0. 29 of 36 confirmed at current resolution. **Decisions changed (4 substantive + 1 v2 roadmap note):** Q3 (rationale softened — wedge is data-stage-aware, not anti-algorithm; MVP behavior unchanged), Differentiator row (same softening), NT-2 (digest hero: single → three-hero carousel: most-rated, most-reviewed, most-Aux'd), SS-3 (invite landing: no → add "X just joined" line, gated by `User.visible_in_just_joined` opt-in flag; default OFF closed-beta cohorts, default ON post-launch), UJ-3 (reviews: inline expand → dedicated `/review/:id` route only, full hero surface, feed shows ~80-char preview), UJ-4 v2-roadmap-note (screenshot image-gen queued for v2 atop the new route). **DM-1 considered then reverted** — Redis feed-page cache from Day 1 was picked then walked back; original p95>200ms trigger preserved. None of the 48 shipped tasks touched; all changes affect unbuilt surfaces. | Product Forge Phase 6+ (CR-002) |
