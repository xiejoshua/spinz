...
- **H4 verdict — Spotify Web API: PARTIALLY VALIDATED, with material caveats.** The endpoints auxd needs as core (auth, profile, currently-playing, recently-played, library, search, album/track lookup) are stable and ToS-compatible for a auxd-shaped product **as long as auxd remains in Spotify Development Mode below 25 users, or applies for Extended Quota Mode**. The endpoints auxd might have *liked* as nice-to-have (`audio-features`, `audio-analysis`, `recommendations`, `related-artists`, 30-second `preview_url` on most tracks) **were removed for new third-party apps on 27 November 2024**. Implication for plan: auxd must build a **social-first** product that derives signal from human ratings + reviews + follow graph, not from acoustic similarity. This actually aligns with the problem statement; it just changes which "Discovery" features are buildable in MVP.
- **Apple Music: defer to v2.** MusicKit JS is workable but the auth dance, developer-token rotation, and weaker public catalog metadata make it a strictly worse MVP choice. Add when (a) >15% of waitlist users are Apple Music-only or (b) auxd becomes platform-neutral on identity.
...
### Endpoint inventory vs auxd needs

| auxd capability | Endpoint(s) | Auth scope(s) needed | Status (2026-Q1 [VERIFY]) |
|---|---|---|---|
...
| Recommendations seeds | `GET /recommendations` | none | **GONE for new apps.** auxd cannot rely on this. |
...
- **Token lifetimes:** Access tokens are 1 hour. Refresh tokens are long-lived (~indefinite in practice) but **rotate on every refresh as of 2023+**. auxd must persist the latest refresh token on every exchange — losing the rotation means the user has to re-consent.
- **Scopes for auxd MVP (minimum viable):** `user-read-email user-read-private user-read-recently-played user-read-currently-playing user-library-read user-top-read playlist-read-private user-follow-read`. Total seven scopes. Spotify's consent screen shows these as a long list — UX research below (sub-problem 2) confirms this is a known friction point but unavoidable.
...
**Implication for auxd:** all background sync jobs MUST budget for ~3 req/sec sustained per app. A 1,000-user beta polling `/recently-played` every 5 minutes = ~3.3 req/sec, right at the edge. Phase 5 must design batching + jitter + a global token-bucket limiter shared across workers.

### ToS — can auxd publicly display Spotify data?
...
2. **No data export/aggregation services without explicit approval.** auxd cannot offer a feature like "download your full listening history as CSV" without a separate review.
3. **No commercial use of audio analysis** — moot for auxd, since the endpoints are gone.
4. **No use of Spotify data to train ML models** — this clause was tightened in 2024. A collaborative filtering recommender trained on `listens` data sourced from Spotify is a grey area; building a graph-walk recommender over auxd's own ratings is clearly safe.
5. **No replicating the Spotify player experience.** auxd must not implement playback controls (skip, seek, queue) within the auxd UI — must deep-link to the Spotify app/web player instead. auxd can show "Now playing" *display* of the current track but not control it. The **Spotify Web Playback SDK** is allowed for embedded playback but **only for Premium users** and counts against rate limits differently.
6. **No commercial monetization without prior approval.** auxd's free + premium model needs sign-off as part of Extended Quota Mode application. Letterboxd-style "premium tier with no ads + extra features" is approvable; data-resale obviously is not.
7. **Branding:** auxd cannot use "Spotify" in the product name, logo, or marketing in a way that implies endorsement. "auxd — for Spotify" is allowed; "auxd Spotify" is not.

**Verdict on ToS:** auxd's stated scope is ToS-compatible. The friction is the Extended Quota Mode review, not the law of the platform.

### Regional catalog differences

Significant. Spotify exposes a `market` (ISO 3166-1 alpha-2) parameter on `/albums`, `/tracks`, `/search`. Without it, "available markets" must be checked client-side. For auxd:
...
- Track ISRC codes and album UPC codes are stable across markets; Spotify track IDs are usually the same but **occasionally diverge for region-specific releases (e.g., Japan-only bonus track editions)**. Treat the Spotify ID as the canonical auxd album ID **only for the canonical market** — MusicBrainz release-group ID is the safer cross-market identifier (see sub-problem 3).
...
| **@spotify/web-api-ts-sdk** (official) | TS/JS | Apache-2.0 | ~1.5k | Official, active since 2023 | **Recommended for any direct-from-browser calls** (which auxd should keep minimal — most calls go through FastAPI). Handles PKCE for you. |
| **Spotify Web Playback SDK** | JS (browser) | proprietary | n/a | Active | Use only if auxd adds an embedded player (out of MVP scope per problem statement). |

**Recommendation: server-side `httpx.AsyncClient` + a thin auxd `SpotifyClient` wrapper.** Reasons: (1) full control over rate limiting and retries; (2) tokens never leave the server for backend-side calls; (3) the API surface is small enough that the SDK adds no leverage; (4) we get clean tracing/observability by owning the client. Pull in `@spotify/web-api-ts-sdk` only for the browser-side login-redirect handler.

### Final H4 verdict

> **H4 (Integration feasibility): VALIDATED with caveats.** The endpoints auxd needs for its *social* core — auth, library, recently-played, search, album/track metadata — are stable, ToS-compatible, and rate-limit-survivable at MVP scale. The endpoints auxd might have wanted for *acoustic* discovery (audio features, recommendations, related artists) are removed for new apps as of 27 Nov 2024 and cannot be relied upon. This forces auxd to lean harder into the social-graph differentiator (which H2 already advocates) and makes the "Recommendations" feature in MVP a social-graph problem, not a content-similarity problem. **Net: H4 is a green light, but with a forced product simplification.**

### Gotchas / caveats

1. **No historical listening backfill.** "Recently played" caps at 50 items / ~24h. auxd cannot replay the "imported my last 5 years of listens" Last.fm onboarding moment from Spotify alone. Workaround: ask users to upload their **Spotify Data Export** (GDPR self-service ZIP, ~30-day turnaround). Decision for plan: include export-importer as MVP or v2?
...
3. **Extended Quota timeline.** If launch needs >25 users on day one, auxd must submit for Extended Quota **before** any go-live announcement. 2–6 week turnaround minimum. Phase 5 plan must put this on the critical path.
4. **Deep-link reliability.** `spotify:album:...` URIs open the native app on mobile if installed; otherwise fall through to `open.spotify.com/album/...`. auxd must always render both.
5. **Country lock-out.** If auxd launches global but a critic in JP reviews a JP-only release, US users see broken playback links. Plan for the "unavailable in your market" empty state.
...
The album-identity problem — "is *Songs in the Key of Life* (1976, US LP) the same record as *Songs in the Key of Life* (2014 remaster, Japan SHM-CD)?" — is the single hardest data-modelling problem auxd faces.
...
| **Discogs** | Strong for physical/collector releases (vinyl, cassette, indie); weak for streaming-era pop | Discogs Release ID (integer) | Decent REST; 60 req/min auth'd | Discogs ToS (somewhat restrictive on bulk use) | Useful for "physical first" album metadata but auxd's casual listeners don't care about pressings. |
...
| **Full catalog mirror** | Fastest reads; arbitrary aggregation queries | 100k+ album seed = ~5GB of metadata; refresh strategy is its own product | v2+, only if auxd scales to >100k DAU or builds genuine catalog-spanning features (trending, charts). |
...
2. **External IDs map:** every auxd `Album` document carries `external_ids: { mbid, spotify_id, apple_id (later), isrc_first_track }`.
3. **Cross-reference at write time:** when auxd first sees a Spotify album, it queries MusicBrainz by Spotify ID (MB stores Spotify URL as a release-level relation). Hit rate: ~85% for releases since 2010, lower for older. Miss → use Spotify ID as canonical; backfill later if MB catches up.
...
**Recommendation: Authlib for the OAuth dance + a 50-line auxd session module that issues HttpOnly cookies with a server-side session id mapped to (user_id, spotify_refresh_token).** Avoid JWT-as-session (the revocation story is worse than session cookies for a single-monolith app).
...
| **RQ** | ~10k | Redis queue, sync | Redis | Simpler than Celery but sync-only. If you want sync, this is the pick — but auxd is async, so prefer arq. |
| **Dramatiq** | ~4k | Redis/Rabbit queue, sync | broker | Niche. Skip. |
| **Cloud-managed (e.g., Inngest, Hatchet)** | varies | managed durable workflows | hosted | Compelling for v2 if auxd needs durable retries + scheduling-as-code. Skip MVP — adds a vendor. |
...
- auxd session lookups (TTL = session lifetime, sliding).
...
auxd's feed query is roughly: *"for each user X, find the most recent N reviews/ratings/listens posted by users that X follows."*
...
| **Bespoke scripts in `migrations/` + a `schema_version` field per document** | **Recommended for auxd.** A 100-line Python harness that diffs current schema_version against target and applies idempotent functions. Encourages "every change is reversible" thinking. |
...
| SEO for album pages (so a auxd album page can rank for "{album name} review") | Strong | Weak; SPA-only is invisible to most crawlers (Google does render JS but slowly and inconsistently) |
...
1. **Album pages are the share-bait surface.** Letterboxd's growth driver is the shareability of film URLs with rich previews. auxd inherits this. SSR + per-route `generateMetadata` is the cheapest path to it.
2. **SEO matters for the long tail.** A auxd album page that ranks #3 for "[obscure album] review" is a discovery channel forever. Spotify Wrapped screenshots don't index; auxd's album pages can.
...
| **Clerk / WorkOS / Stytch** | n/a | Skip — auxd's identity layer is just Spotify; don't pay for vendor managed identity. |
...
|---|---|---|---|
| **shadcn/ui + Radix Primitives + Tailwind** | n/a (shadcn is copy-paste, not npm) | Owned components | **Recommended.** Maximum customisation for a brand-forward product like auxd. Component code lives in your repo. |
...
| **MUI** | ~93k | Library | Material design baked in — wrong aesthetic for auxd. Skip. |
| **Park UI / Ark UI** | smaller | Headless | Promising; less mature than shadcn. Skip MVP. |

auxd needs a strong visual identity (it's a Letterboxd competitor — aesthetics are part of the moat). shadcn lets the team customise to that identity without fighting a component library.
...
- **Always cache CDN URLs in Mongo** so auxd isn't re-fetching the same album object from Spotify just to know the cover URL.
- **Self-hosting cover art:** unnecessary unless auxd moves off Spotify catalog.
...
With Spotify's `/recommendations` removed (sub-problem 1), auxd cannot use Spotify's recommender as a fallback. The recommendation surface must be built from auxd's own data: ratings, reviews, listens, and the follow graph.
...
| **Tag/genre propagation** | "Users who follow indie-folk on auxd tend to like X." | Mongo aggregation | Easy add. |
...
| **Embedding-based / vector similarity** | "Embed each user and album in latent space." | `sentence-transformers` for review-text embeddings; Mongo Atlas Vector Search for retrieval. | v2+. auxd reviews are text — embedding reviews into a "taste vector" is a genuine differentiator. |
...
| **Surprise** | ~6.4k | Quiet since 2023 | BSD-3 | Easy to learn; slower; explicit ratings (which auxd has). Good for a quick prototype. |
...
Add **`implicit` + ALS** in v2 once auxd has >5k ratings, to power a fourth strip ("You might like...") that uses CF.
...
- Sparse ratings = bad CF signal. auxd won't have enough data until 5k+ ratings; shipping CF too early produces terrible recommendations and damages trust.
...
| **Meilisearch** | ~46k | Self-host (small Rust binary) or Meilisearch Cloud | Excellent | Strong typo tolerance, great DX. Adds a new service. Worth it if auxd outgrows Atlas Search. |
...
| **Spotify Search as fallback** | n/a | Free (subject to rate limit) | Reasonable | **Always on for albums.** If a user searches "the new clairo album" and auxd doesn't have a cached album doc, fall through to Spotify search, then cache the result. |
...
- **Catalog passthrough:** when a user's query yields no auxd album result, hit Spotify Search, cache the top 10 results into the `albums` collection, return them. This is the "we always have the album, even if we've never seen it before" trick.
...
At MVP, auxd's "feed" is a paginated list of recent activity from followees. Real-time push (live new-review notifications) is **nice to have, not load-bearing**.
...
| **WebSockets** | Medium; bidirectional | Universal | Overkill. auxd has no client→server real-time need at MVP. |
...
- Skip WebSockets until auxd adds live chat or co-listening features (out of scope).
...
The single most important integration in auxd needs its own dashboard:
...
5. **OAuth redirect domains.** Three Spotify apps: `auxd-dev`, `auxd-staging`, `auxd-prod`. Three Apple Music keys when added. Document in `infra/spotify-apps.md`.
