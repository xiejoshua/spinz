...
- [x] **T003 — Monorepo scaffolding (pnpm + uv + workspaces)** *(verified 2026-05-22: pnpm install ✓, uv sync ✓, both dev servers boot ✓, frontend resolves @spinz/shared-types via workspace:* protocol — implementation-log.md checkpoint #3)*
      ...
      Done: `pnpm install` succeeds at root; `uv sync` succeeds in `apps/api`; `pnpm --filter @spinz/web dev` and `uv run --project apps/api uvicorn ...` both boot.
...
- [ ] **T006 — Provision Fly.io + Vercel projects**
      ...
      Deps: T003
      Refs: plan §17.1, §17.2
      Description: `flyctl apps create spinz-api`; configure two-process layout (api + worker); link `apps/api/fly.toml`. `vercel init` linking `apps/web` to a Vercel project. Domain `spinz.app` and `api.spinz.app` DNS configured.
      Done: empty hello-world deploys from each host land at expected URLs.

- [ ] **T007 — Configure Postmark + Sentry + PostHog + secrets**
      Paths: apps/api/src/spinz_api/settings.py, docs/infra.md
      Size: S
      Deps: T005, T006
      Refs: plan §15, §17.6
      Description: Create accounts; configure DNS records for Postmark (DKIM/SPF/DMARC on spinz.app). Set secrets via `fly secrets set` (MONGODB_URI, REDIS_URL, SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SESSION_HMAC_KEY, TOKEN_ENCRYPTION_KEY, POSTMARK_API_KEY, VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY, SENTRY_DSN, POSTHOG_API_KEY). Self-host PostHog as a separate Fly app (single container, M0).
...
- [ ] **T010 — Synthetic monitoring + alerting**
      ...
      Deps: T009
      Refs: plan §15.5
      Description: GitHub Actions cron every 15min curls `https://spinz.app` and `https://api.spinz.app/healthz`, posts to Discord webhook on failure.
...
- [ ] **T011 — FastAPI app skeleton + healthz**
      Paths: apps/api/src/spinz_api/main.py, apps/api/src/spinz_api/settings.py
      Size: S
      Deps: T003, T005
...
- [ ] **T012 — Beanie + MongoDB connection**
      Paths: apps/api/src/spinz_api/db.py, apps/api/tests/integration/test_db_connection.py
      Size: S
      Deps: T011
      Refs: plan §3
...
- [ ] **T013 — Redis connection + arq worker scaffold + fail-mode wiring**
      Paths: apps/api/src/spinz_api/redis_client.py, apps/api/src/spinz_api/workers/main.py, apps/api/Dockerfile, apps/api/tests/integration/test_redis_fail_modes.py
      Size: M
      Deps: T011, T005
...
- [ ] **T014 — `lib/resilience` (retry + timeout + circuit breaker)**
      Paths: apps/api/src/spinz_api/lib/resilience.py, apps/api/tests/unit/test_resilience.py
      Size: M
      Deps: T011
...
- [ ] **T015 — `lib/observability` (structured logging + PostHog client)**
      Paths: apps/api/src/spinz_api/lib/observability.py, apps/api/tests/unit/test_observability.py
      Size: M
      Deps: T011, T013
...
- [ ] **T016 — `lib/visibility` (single source of truth for can_read)**
      Paths: apps/api/src/spinz_api/lib/visibility.py, apps/api/tests/unit/test_visibility.py
      Size: M
      Deps: T012
...
- [ ] **T017 — Encrypted token storage utility**
      Paths: apps/api/src/spinz_api/lib/secrets.py, apps/api/tests/unit/test_secrets.py
      Size: S
      Deps: T011
...
- [ ] **T018 — KSUID ID generator**
      Paths: apps/api/src/spinz_api/lib/ids.py, apps/api/tests/unit/test_ids.py
      Size: XS
      Deps: T011
      Refs: plan §3
...
- [ ] **T019 — Session middleware (HMAC cookie + CSRF)**
      Paths: apps/api/src/spinz_api/lib/sessions.py, apps/api/src/spinz_api/middleware.py
      Size: M
      Deps: T011, T017
...
- [ ] **T020 — Rate-limit middleware (Redis-backed, fail-open)**
      Paths: apps/api/src/spinz_api/lib/rate_limit.py, apps/api/tests/integration/test_rate_limit.py
      Size: M
      Deps: T013, T015
...
- [ ] **T021 — User Document + handle policy fields**
      Paths: apps/api/src/spinz_api/modules/auth/models.py, apps/api/tests/unit/test_user_model.py
      Size: M
      Deps: T012, T018
...
- [ ] **T022 — Album Document + identity fields**
      Paths: apps/api/src/spinz_api/modules/albums/models.py
      Size: M
      Deps: T012, T018
...
- [ ] **T023 — DiaryEntry + Review + ReviewLike + ReviewEditHistory Documents**
      Paths: apps/api/src/spinz_api/modules/diary/models.py, apps/api/src/spinz_api/modules/reviews/models.py
      Size: M
      Deps: T021, T022
...
- [ ] **T024 — Backlog + BacklogItem Documents**
      Paths: apps/api/src/spinz_api/modules/backlog/models.py
      Size: S
      Deps: T021, T022
      Refs: plan §3
...
- [ ] **T025 — Follow + FollowRequest + Block Documents**
      Paths: apps/api/src/spinz_api/modules/social/models.py
      Size: S
      Deps: T021
...
- [ ] **T026 — Report + Notification + NotificationPreferences + FailedEmail Documents**
      Paths: apps/api/src/spinz_api/modules/moderation/models.py, apps/api/src/spinz_api/modules/notifications/models.py
      Size: M
      Deps: T021
...
- [ ] **T027 — JustFinishedPrompt + SuggestedFollow + CriticSeed Documents**
      Paths: apps/api/src/spinz_api/modules/prompts/models.py, apps/api/src/spinz_api/modules/seeding/models.py
      Size: M
      Deps: T021, T022
      Refs: plan §3
...
- [ ] **T028 — OpenAPI → TS type codegen pipeline**
      ...
      Done: a backend schema change produces a TS diff in shared-types; frontend consumes from `@spinz/shared-types`.

- [ ] **T029 — Pydantic settings + env validation**
      Paths: apps/api/src/spinz_api/settings.py
      Size: S
      Deps: T011
      Refs: plan §17.2
...
- [ ] **T030 — Migration runner skeleton**
      Paths: apps/api/src/spinz_api/migrations/__init__.py, apps/api/src/spinz_api/migrations/runner.py
      Size: S
      Deps: T012
...
- [ ] **T031 — Next.js 15 App Router skeleton**
      ...
      Done: `pnpm --filter @spinz/web dev` serves the page.
...
- [ ] **T033 — TanStack Query setup + API client**
      ...
      Deps: T031, T028
      Refs: plan §7.3
      Description: QueryClient provider; typed fetch wrapper consuming `@spinz/shared-types`; React Query DevTools in dev. Standard `useApiQuery` and `useApiMutation` helpers.
...
- [ ] **T041 — MusicProvider + CatalogProvider protocols**
      Paths: apps/api/src/spinz_api/providers/base.py
      Size: S
      Deps: T014, T015
...
- [ ] **T042 — Spotify provider — contract tests (FAIL state) [TEST-FIRST]**
      ...
      Description: Contract tests against Spotify's actual API (Development Mode quota) covering all methods Spinz uses. Tests run as `pytest -m contract` and are excluded from normal CI but run nightly + on main. At this stage they MUST FAIL (no implementation yet) — that's the test-first pattern.
      Done: pytest collects 12+ tests; all fail with import/implementation errors (expected).

- [ ] **T043 — Spotify provider — implementation**
      Paths: apps/api/src/spinz_api/providers/spotify/__init__.py, apps/api/src/spinz_api/providers/spotify/client.py, apps/api/src/spinz_api/providers/spotify/oauth.py, apps/api/src/spinz_api/providers/spotify/mappings.py
      Size: L
...
- [ ] **T044 — Spotify provider — token refresh + revoke handling**
      Paths: apps/api/src/spinz_api/providers/spotify/oauth.py, apps/api/tests/integration/test_spotify_oauth.py
      Size: M
      Deps: T043
...
- [ ] **T049 — MusicBrainz catalog provider — implementation**
      Paths: apps/api/src/spinz_api/providers/musicbrainz/__init__.py, apps/api/src/spinz_api/providers/musicbrainz/client.py
      Size: M
      Deps: T048, T014
...
- [ ] **T050 — Resilience transport — httpx custom Transport**
      Paths: apps/api/src/spinz_api/lib/http_transport.py
      Size: M
      Deps: T014
...
- [ ] **T051 — Provider observability instrumentation**
      Paths: apps/api/src/spinz_api/providers/spotify/client.py, apps/api/src/spinz_api/providers/musicbrainz/client.py
      Size: S
      Deps: T043, T049, T015
...
- [ ] **T052 — Provider error taxonomy**
      Paths: apps/api/src/spinz_api/providers/errors.py
      Size: S
      Deps: T041
      Refs: plan §5
...
- [ ] **T053 — Email/password signup + login endpoints**
      Paths: apps/api/src/spinz_api/modules/auth/routes.py, apps/api/src/spinz_api/modules/auth/service.py, apps/api/tests/integration/test_auth_email.py
      Size: M
      Deps: T021, T019, T020
      Refs: US-A1; FR-001
...
- [ ] **T054 — Spotify OAuth signup/login shortcut**
      Paths: apps/api/src/spinz_api/modules/auth/spotify_oauth.py, apps/api/tests/integration/test_spotify_signup.py
      Size: M
      Deps: T053, T043, T044
...
- [ ] **T055 — Connect-Spotify-later flow (post-signup)**
      Paths: apps/api/src/spinz_api/modules/auth/service.py, apps/api/tests/integration/test_connect_spotify.py
      Size: M
      Deps: T054, T044
...
- [ ] **T056 — Disconnect Spotify (immutability)**
      Paths: apps/api/src/spinz_api/modules/auth/service.py, apps/api/tests/integration/test_disconnect_spotify.py
      Size: S
      Deps: T055
...
- [ ] **T057 — Handle change policy + reserved-squat list**
      Paths: apps/api/src/spinz_api/modules/auth/handle_service.py, apps/api/data/reserved_handles.txt, apps/api/tests/unit/test_handle_policy.py
      Size: M
      Deps: T021
...
- [ ] **T058 — Account deletion (30-day grace) + cascade**
      Paths: apps/api/src/spinz_api/modules/auth/service.py, apps/api/src/spinz_api/workers/deletion_cascade.py, apps/api/tests/integration/test_account_deletion.py
      Size: M
...
- [ ] **T059 — Logout (current session + logout-all-devices)**
      Paths: apps/api/src/spinz_api/modules/auth/service.py
      Size: XS
      Deps: T053, T019
      Refs: NFR Security
...
- [ ] **T060 — Handle redirect routing**
      Paths: apps/web/src/middleware.ts, apps/api/src/spinz_api/modules/auth/routes.py
      Size: S
      Deps: T057
      Refs: FR-029
...
- [ ] **T063 — Album identity normalization service**
      Paths: apps/api/src/spinz_api/modules/albums/identity.py, apps/api/tests/unit/test_album_identity.py
      Size: M
      Deps: T022, T043, T049
...
- [ ] **T064 — Album cache + TTL refresh worker**
      Paths: apps/api/src/spinz_api/workers/album_cache_refresh.py
      Size: S
      Deps: T063, T013
...
- [ ] **T065 — MBID reconciliation worker**
      Paths: apps/api/src/spinz_api/workers/mbid_reconcile.py
      Size: S
      Deps: T063, T049
...
- [ ] **T066 — Edition aggregation (release-group → editions)**
      Paths: apps/api/src/spinz_api/modules/albums/editions.py, apps/api/tests/unit/test_editions.py
      Size: M
      Deps: T063
...
- [ ] **T067 — Album detail endpoint**
      Paths: apps/api/src/spinz_api/modules/albums/routes.py, apps/api/tests/integration/test_album_detail.py
      Size: M
      Deps: T063, T066, T016
      Refs: US-F1; FR-010
...
- [ ] **T069 — Search endpoint (Atlas + Spotify fallback merge)**
      Paths: apps/api/src/spinz_api/modules/search/routes.py, apps/api/src/spinz_api/modules/search/service.py, apps/api/tests/integration/test_search.py
      Size: M
      Deps: T068, T043
...
- [ ] **T073 — Diary log endpoint (Award-bool included)**
      Paths: apps/api/src/spinz_api/modules/diary/routes.py, apps/api/src/spinz_api/modules/diary/service.py, apps/api/tests/integration/test_diary_logging.py
      Size: M
      Deps: T023, T021, T063
...
- [ ] **T074 — Diary read endpoint (chronological diary)**
      Paths: apps/api/src/spinz_api/modules/diary/routes.py
      Size: S
      Deps: T073, T016
      Refs: US-E2; FR-007
...
- [ ] **T075 — Diary edit / delete endpoints**
      Paths: apps/api/src/spinz_api/modules/diary/routes.py, apps/api/tests/integration/test_diary_edit.py
      Size: M
      Deps: T073
      Refs: US-B5; FR-004
...
- [ ] **T079 — Prefill album from Spotify recently-played (Log sheet)**
      Paths: apps/web/src/components/log-sheet/album-prefill.tsx, apps/api/src/spinz_api/modules/diary/routes.py
      Size: M
      Deps: T077, T043
...
- [ ] **T085 — Review create endpoint**
      Paths: apps/api/src/spinz_api/modules/reviews/routes.py, apps/api/src/spinz_api/modules/reviews/service.py
      Size: M
      Deps: T023, T073
      Refs: US-C1; FR-011
...
- [ ] **T086 — Review edit + audit log**
      Paths: apps/api/src/spinz_api/modules/reviews/service.py, apps/api/tests/integration/test_review_edit.py
      Size: M
      Deps: T085
...
- [ ] **T087 — Review delete (with cascading reaction cleanup)**
      Paths: apps/api/src/spinz_api/modules/reviews/service.py
      Size: S
      Deps: T085
      Refs: DM-5
...
- [ ] **T088 — Like-review endpoint (idempotent toggle)**
      Paths: apps/api/src/spinz_api/modules/reviews/likes_service.py, apps/api/tests/unit/test_review_likes.py
      Size: M
      Deps: T085
...
- [ ] **T089 — List reviews endpoint with sort**
      Paths: apps/api/src/spinz_api/modules/reviews/routes.py, apps/api/tests/unit/test_review_sort.py
      Size: M
      Deps: T088, T016
...
- [ ] **T095 — Backlog CRUD endpoints**
      Paths: apps/api/src/spinz_api/modules/backlog/routes.py, apps/api/src/spinz_api/modules/backlog/service.py
      Size: M
      Deps: T024, T021
...
- [ ] **T096 — Auto-remove-on-log behavior**
      Paths: apps/api/src/spinz_api/modules/diary/service.py
      Size: S
      Deps: T073, T095
      Refs: US-D3
...
- [ ] **T100 — Backlog → Log conversion event**
      Paths: apps/api/src/spinz_api/modules/diary/service.py, apps/web/src/lib/analytics.ts
      Size: XS
      Deps: T096
...
- [ ] **T101 — Follow / unfollow endpoints**
      Paths: apps/api/src/spinz_api/modules/social/routes.py, apps/api/src/spinz_api/modules/social/service.py
      Size: M
      Deps: T025, T021
      Refs: US-E1; FR-008
...
- [ ] **T102 — Block endpoint (+ cascade-resolve follow)**
      Paths: apps/api/src/spinz_api/modules/social/service.py, apps/api/tests/integration/test_blocks.py
      Size: M
      Deps: T101
...
- [ ] **T103 — Friends-who-rated-and-awarded endpoint**
      Paths: apps/api/src/spinz_api/modules/feed/service.py
      Size: M
      Deps: T101, T073
      Refs: US-E4; FR-010
...
- [ ] **T104 — Suggested-follow precompute worker**
      Paths: apps/api/src/spinz_api/workers/suggestions_job.py
      Size: L
      Deps: T101, T073
      Refs: US-E5; FR-016
...
- [ ] **T105 — Suggested-follow API + dismiss endpoint**
      Paths: apps/api/src/spinz_api/modules/social/routes.py
      Size: S
      Deps: T104
      Refs: US-E5
...
- [ ] **T106 — Home feed endpoint (fan-out-on-read + weighting)**
      Paths: apps/api/src/spinz_api/modules/feed/routes.py, apps/api/src/spinz_api/modules/feed/service.py, apps/api/tests/integration/test_feed.py
      Size: L
...
- [ ] **T115 — Auto-import worker (30-day Spotify history)**
      Paths: apps/api/src/spinz_api/workers/spotify_import.py, apps/api/tests/integration/test_spotify_import.py
      Size: M
      Deps: T043, T073
...
- [ ] **T117 — Critic-seed batch precompute algorithm (backend, scheduled)**
      Paths: apps/api/src/spinz_api/modules/seeding/service.py, apps/api/tests/unit/test_seeding_algorithm.py
      Size: M
      Deps: T027, T104
...
- [ ] **T117a — Critic-seed SYNCHRONOUS card-ordering for onboarding** *(added in Phase 5C Revision)*
      Paths: apps/api/src/spinz_api/modules/seeding/onboarding_service.py, apps/api/tests/unit/test_onboarding_card_ordering.py
      Size: M
      Deps: T117, T115, T163
...
- [ ] **T119 — Onboarding step 5: Land on feed (non-empty guarantee)**
      Paths: apps/web/src/app/(onboarding)/complete/page.tsx, apps/api/src/spinz_api/modules/feed/service.py
      Size: S
      Deps: T106, T118
      Refs: US-A5
...
- [ ] **T122 — Last.fm import (Should-Have)**
      Paths: apps/api/src/spinz_api/workers/lastfm_import.py, apps/web/src/app/(onboarding)/lastfm-import/page.tsx
      Size: M
      Deps: T039, T115
      Refs: US-H1
...
- [ ] **T123 — Spotify polling worker**
      Paths: apps/api/src/spinz_api/workers/spotify_poll.py
      Size: L
      Deps: T043, T027
      Refs: US-B6; FR-026
...
- [ ] **T124 — Just-finished detection heuristic**
      Paths: apps/api/src/spinz_api/modules/prompts/detection.py, apps/api/tests/unit/test_detection.py
      Size: M
      Deps: T123
      Refs: US-B6
...
- [ ] **T125 — JustFinishedPrompt lifecycle endpoints**
      Paths: apps/api/src/spinz_api/modules/prompts/routes.py, apps/api/tests/integration/test_prompts_lifecycle.py
      Size: M
      Deps: T027, T124
...
- [ ] **T126 — Quiet hours enforcement (prompts + push)**
      Paths: apps/api/src/spinz_api/modules/notifications/quiet_hours.py
      Size: S
      Deps: T026
      Refs: FR-026; NT-3
...
- [ ] **T128 — Web push notification dispatch (for auto-prompt)**
      Paths: apps/api/src/spinz_api/modules/notifications/adapters/web_push.py
      Size: M
      Deps: T008, T126
      Refs: US-B6; FR-026
...
- [ ] **T131 — Notification dispatcher core**
      Paths: apps/api/src/spinz_api/modules/notifications/dispatcher.py, apps/api/tests/unit/test_dispatcher.py
      Size: L
      Deps: T026
      Refs: FR-012; plan §8
...
- [ ] **T132 — `is_notifiable` predicate**
      Paths: apps/api/src/spinz_api/modules/notifications/dispatcher.py
      Size: S
      Deps: T131
...
- [ ] **T133 — Coalescer + rate limiter**
      Paths: apps/api/src/spinz_api/modules/notifications/coalescer.py, apps/api/tests/integration/test_coalescer.py
      Size: L
      Deps: T131, T013
...
- [ ] **T134 — InApp notification adapter**
      Paths: apps/api/src/spinz_api/modules/notifications/adapters/in_app.py
      Size: S
      Deps: T131, T026
      Refs: FR-012
...
- [ ] **T135 — Email notification adapter (Postmark) + failure-mode wiring**
      Paths: apps/api/src/spinz_api/modules/notifications/adapters/email.py, apps/api/src/spinz_api/modules/notifications/models.py (FailedEmail Document), apps/api/src/spinz_api/templates/email/*
      Size: M
      Deps: T131, T007, T026
...
- [ ] **T137 — Notification types (all 18 actives)**
      Paths: apps/api/src/spinz_api/modules/notifications/types.py, apps/api/tests/integration/test_notification_types.py
      Size: M
...
- [ ] **T138 — Weekly digest job**
      Paths: apps/api/src/spinz_api/workers/digest_dispatch.py, apps/api/tests/integration/test_weekly_digest.py
      Size: L
      Deps: T135, T106
...
- [ ] **T143 — Coalesce review.liked into weekly digest aggregate**
      Paths: apps/api/src/spinz_api/workers/digest_dispatch.py
      Size: S
      Deps: T138
      Refs: NT-4
...
- [ ] **T144 — Notification rate-limit alerting**
      Paths: apps/api/src/spinz_api/lib/observability.py, .github/workflows/observability-alerts.yml
      Size: XS
      Deps: T015, T133
...
- [ ] **T146 — Avatar upload pipeline**
      Paths: apps/api/src/spinz_api/modules/auth/routes.py, apps/api/src/spinz_api/lib/storage.py
      Size: S
      Deps: T021
      Refs: US-G1
...
- [ ] **T148 — Private profile → follow-request queue**
      Paths: apps/api/src/spinz_api/modules/social/service.py, apps/web/src/components/social/follow-requests.tsx
      Size: M
      Deps: T101, T147
      Refs: US-G2, US-H3
...
- [ ] **T150 — Settings → Account (email, password change)**
      Paths: apps/web/src/app/(app)/settings/account/page.tsx, apps/api/src/spinz_api/modules/auth/service.py
      Size: M
      Deps: T053, T059
      Refs: NFR Security
...
- [ ] **T153 — Data export job**
      Paths: apps/api/src/spinz_api/workers/gdpr_export.py, apps/api/tests/integration/test_gdpr_export.py
      Size: L
...
- [ ] **T154 — GDPR audit log collection**
      Paths: apps/api/src/spinz_api/lib/audit.py
      Size: S
      Deps: T012
      Refs: NFR Compliance
...
- [ ] **T155 — Report submission endpoint**
      Paths: apps/api/src/spinz_api/modules/moderation/routes.py
      Size: M
      Deps: T026, T101
      Refs: US-G4; FR-014
...
- [ ] **T156 — Daily moderation log-scan**
      Paths: apps/api/src/spinz_api/workers/moderation_scan.py
      Size: M
      Deps: T155
      Refs: US-G4
...
- [ ] **T157 — Report acknowledgment notification (N-012)**
      Paths: apps/api/src/spinz_api/modules/notifications/types.py
      Size: XS
      Deps: T137, T155
...
- [ ] **T158 — Admin notes / read-only flag visibility**
      Paths: apps/api/src/spinz_api/modules/auth/models.py
      Size: XS
      Deps: T021
      Refs: US-G4
...
- [ ] **T159 — Suspended account UX**
      Paths: apps/api/src/spinz_api/middleware.py, apps/web/src/app/suspended/page.tsx
      Size: S
      Deps: T155
      Refs: US-G4
...
- [ ] **T163 — Genre signature computation**
      Paths: apps/api/src/spinz_api/modules/seeding/genre_signature.py
      Size: M
      Deps: T117, T073
      Refs: T117 algorithm
...
- [ ] **T164 — Mutual-taste suggestions algorithm**
      Paths: apps/api/src/spinz_api/modules/seeding/mutual_taste.py
      Size: M
      Deps: T163
...
- [ ] **T167 — Album merge / report-wrong-album**
      Paths: apps/api/src/spinz_api/modules/albums/admin.py, apps/web/src/components/album-detail/report-wrong.tsx
      Size: M
      Deps: T067
      Refs: US-H2
...
- [ ] **T169 — Pull-more-history (Should-Have)**
      Paths: apps/web/src/app/(app)/settings/integrations/page.tsx, apps/api/src/spinz_api/workers/spotify_import.py
      Size: M
      Deps: T115
