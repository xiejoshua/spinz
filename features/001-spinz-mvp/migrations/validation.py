"""
Spinz MVP — Migration 001_initial (validation)
...
async def validate_pre(db: AsyncIOMotorDatabase, results: CheckResult) -> None:
    ...
    print("[Pre-migration validation: cluster should be empty of Spinz collections]")
    existing = await db.list_collection_names()
    spinz_present = [c for c in existing if c in EXPECTED_COLLECTIONS]
    if spinz_present:
        results.fail(
            "Pre-migration empty check",
            f"Found existing Spinz collections: {spinz_present}. Cluster is not empty.",
        )
    else:
        results.ok("Pre-migration empty check: no Spinz collections present")
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="Spinz MVP — migration validation")
