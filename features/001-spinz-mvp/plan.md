# Plan: Spinz MVP — Technical Blueprint
...
```python
# apps/api/src/spinz_api/modules/diary/models.py
...
```python
# apps/api/src/spinz_api/lib/visibility.py
...
```python
# apps/api/src/spinz_api/lib/rate_limit.py
async def endpoint(
    request: Request,
...
Backend services in `apps/api/src/spinz_api/modules/auth/` (or new `integrations/` module):
...
```python
# apps/api/src/spinz_api/providers/base.py
...
- Profile URL share (`spinz.app/@handle`)
...
GitHub Actions cron every 15 minutes: `curl https://spinz.app` → assert 200 + < 500ms. Failure → ping founder via Discord webhook. Pingdom / BetterStack deferred to v1.x.
...
- Single app `spinz-api` deployed via `flyctl deploy` on push to `main` (GitHub Actions).
...
- Domain `spinz.app` (founder-registered pre-launch).
- Apex → Vercel; `api.spinz.app` → Fly.io.
...
- Postmark with `spinz.app` domain DKIM/SPF/DMARC configured pre-launch.
