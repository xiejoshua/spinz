# Cold-Start Seeding Strategy: Spinz MVP
...
A roster of ~50 real-world music critics, curators, and tastemakers given Spinz accounts and a soft incentive to use them. **This must be alive before public-access wave.** Without it, Spinz launches as an empty room.
...
- Active currently (not retired, not dormant) — must plausibly post on Spinz within the launch month.
...
2. **Personal cold outreach** (DM/email): one-paragraph pitch, "early access + 6 months Spinz Pro free + founder Slack access".
...
5. **Soft incentive at launch**: featured-critic badge, "top 10 reviews this week" surface, future revenue share if/when Spinz monetizes.
...
- Every user gets a personal invite link: `spinz.app/i/{handle}` (vanity, redirects to signup with referrer tracking).
- The link displays the inviter's profile card on the landing page: "Casey invited you to Spinz" + their 5 recent ratings.
...
- Both parties see a "Casey joined Spinz" notification when the invitee logs their first album (closes the loop, encourages mutual-follow).
