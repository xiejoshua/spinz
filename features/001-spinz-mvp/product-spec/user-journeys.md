...
> Entry point: spinz.app landing page. Exit point: home feed populated, ≥1 album logged. Target completion time: **<3 minutes**.

| Step | User action | System response | Emotion | Notes |
|---|---|---|---|---|
| 1 | Lands on `spinz.app` from a friend's invite link | Shows: "Casey invited you to Spinz" hero + Casey's 5 recent ratings + "Get started with Spotify" big button | 😊 curious | Social proof primes commitment |
...
| 3 | Approves Spotify scopes | Redirects back to Spinz with auth code; exchange for tokens; account auto-created using Spotify display name and email | 😊 quick | Auto-create account avoids form-filling tax |
...
> Entry point: Casey finishes an album on Spotify and either opens Spinz within ~15 minutes OR has auto-prompt push enabled. Exit point: diary entry created from the prompt. Target: ≥30% of Spotify-connected sessions trigger an auto-prompt log within 3 months of connect.

| Step | User action | System response | Emotion | Notes |
|---|---|---|---|---|
| 1 | Casey finishes "GNX" on Spotify | Spinz background polling detects finished album within 5–15 min via Spotify recently-played | (background) | Polling cadence: 5 min while user has active app session in last 24h, 15 min otherwise |
| 2 | Casey opens Spinz (or push lands if opted-in) | A `JustFinishedPrompt` is in `pending` state; the home feed renders a hero prompt card at the top: "Just listened to GNX — log it?" with primary "Log" CTA and secondary "Not now" + "⋮ menu" | 😊 mild surprise | Hero prompt is dismissable via swipe-up or "Not now" |
...
| Step | User action | System response | Emotion | Notes |
|---|---|---|---|---|
| 1 | Opens Spinz (app already running or pinned tab) | App resumes on last surface (or home if cold-start) | 😊 |
...
> Entry point: open Spinz home tab. Exit point: user opens Spotify to listen to an album they discovered. Target conversion: 1 in 4 sessions ends in an outbound Spotify play.
...
| (User listens to album on Spotify, returns to Spinz to log later) |
...
| 5 | Later, after listening, returns to Spinz | (Some hours/days later) |
...
| Step | User action | System response | Emotion | Notes |
|---|---|---|---|---|
| 1 | Opens Spinz, taps "+" Log | Bottom sheet opens with prefilled album (or Maya searches) | 😊 |
...
| 6 | Taps "Share" CTA in toast | OS share sheet opens with prefilled URL (`spinz.app/r/{review_id}`) and OG image preview | 😊 amplifies |
