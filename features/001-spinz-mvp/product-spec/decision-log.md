# Decision Log: Spinz MVP

> Feature: `001-spinz-mvp` | Phase: 2 (Product Spec) | Date: 2026-05-21
...
| 15 | Deluxe / remastered editions: separate album entries or merged under release-group? | **Merge under MusicBrainz release-group MBID; surface an "Edition" selector on the album detail page.** One canonical album entry per release group; Edition chip near the cover shows "All editions" with a dropdown for Standard / Deluxe / Bonus / etc. Ratings, reviews, and Awards aggregate across all editions. | Letterboxd-style merge for casual users; the Edition selector preserves power-user granularity without splitting the catalog. Cascade implications: search, dedup, recs, analytics all key off the release-group MBID. | LOCKED (v1.2) |
...
| 22 | Spotify OAuth scopes — which are essential vs nice-to-have at MVP? | **Essential at MVP:** `user-read-recently-played` (auto-import + just-finished detection), `user-read-currently-playing` (just-finished detection — confirms album completion), `user-library-read` (read user's saved albums for backlog hints). **Nice-to-have, request lazily:** `playlist-read-private` (only if Lists ever ingest from playlists; deferred). Request scopes in two rounds if needed — minimal at signup, extend on first feature use. | Reduces consent-screen friction; Spotify's docs favor incremental authorization. Avoids users seeing a long scope list and bouncing. | LOCKED (v1.2) |
...
| Pricing tier | **No paid tier at launch**. Future Spinz Pro ($19/yr) + Patron ($39/yr) documented in spec as future-state. | Per metrics-roi.md: defer until WAL ≥ 1,000 and D30 ≥ 10% |
...
| **Terminology — "Award" vs "Like" semantic split** (refined in Revision #3, 2026-05-21; supersedes the unified Award terminology from Revision #1) | Two distinct concepts, two distinct names: <br> • **Award (🏅)** = a deliberate personal-curation signal on your OWN content. Boolean on DiaryEntry: "this album is one of my standouts in MY diary." Rare and intentional. <br> • **Like (👍)** = a lightweight social engagement signal on OTHER users' content. Counter on Review: number of people who appreciated this user's review. Common and fast. <br> Lists are deferred to v2 (per decision #7), so no Award-or-Like split applies there at MVP. | Letterboxd model: "Like" (heart) on your own log is personal-favorite; "Like" on a review is social. We had unified both under "Award" in R1; founder split them in R3 to be precise about self-directed-curation vs other-directed-engagement. Two icons (🏅 for Award, 👍 for Like) reinforce the distinction in UI. |
...
| **Onboarding edge case — Spotify OAuth email matches existing email/password account** *(added in Phase 5C, 2026-05-22 — D-002 from pre-impl-review)* | When Spotify OAuth returns an email matching an existing ACTIVE Spinz account, do NOT silently auto-link. Show a "Connect Spotify to your existing account?" page that requires the user to authenticate via their existing Spinz password first. On password-success, attach the `MusicProvider` Spotify sub-document to the existing User. If the email matches a deleted-status account, fall through to the rule above. | Silent auto-linking creates security risks (someone with access to a Spotify account auto-merging into a Spinz account they don't own). Requiring password re-auth confirms identity. |
...
| NT-4 | N-004 (review.awarded) — in-app or aggregated into digest only? | **Both.** In-app for immediate feedback (low volume per user); digest aggregates ("your reviews got 12 awards this week") for absent users. |
...
| SS-5 | Off-ramp for a seed who wants to leave the program? | **Standard account, badge removed silently.** No public "Critic X has left Spinz" announcement. |
...
| DM-3 | Reactions on Reviews (awards count) — feature-flag the UI? | **Ship visible.** Already in the data model; gating for vanity is over-engineering. Volume will be low at MVP; that's fine. |
...
| DM-5 | Soft-deleted DiaryEntry: should reactions cascade? | **Yes — cascade.** Don't leave orphan award counts pointing at deleted entries. |
...
| 2026-05-21 | v1.1: Revision #1 in Phase 3 | Founder elevated Lists into MVP scope; enabled auto-prompt for just-finished; made Spotify connect explicitly skippable; renamed Liked/Heart → Awarded/Award across all docs | Product Forge Phase 3 |
...
| 2026-05-21 | v1.3: Revision #3 in Phase 3 | Removed soft "say more" prompt for short reviews; split unified "Award" into "Award" (self-directed personal signal) + "Like" (other-directed social engagement); added Likes on reviews + Most Liked sort; reverted Lists from MVP back to v2 (de-elevated R1's elevation) | Product Forge Phase 3 |
