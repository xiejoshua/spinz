...
- **Edge: Spotify-OAuth email matches existing email/password account (Phase 5C decision):** Given Spotify OAuth returns an email matching an existing ACTIVE Spinz account, When the OAuth callback fires, Then I see a "Connect Spotify to your existing account?" page that requires my existing Spinz password to confirm identity. On confirm, the Spotify `MusicProvider` sub-doc attaches to the existing User. Silent auto-link is NEVER performed.

### S-A2: Optionally connect Spotify and auto-import recent listens
**As Casey,** I want the option to connect Spotify and auto-import my last 30 days, so my first session shows something real — but I also want to be able to skip Spotify entirely and still use Spinz fully.
...
- Given a `JustFinishedPrompt` is `pending`, When I next open Spinz, Then the home feed shows a hero prompt card at the top: "Just listened to {album} — log it?" with primary "Log" CTA and secondary "Not now"/"⋮ menu" options.
...
**As Maya,** I want to import my Last.fm scrobbling history if I prefer not to connect Spotify, so I can join Spinz without OAuth.
