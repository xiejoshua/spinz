...
| **In-app messaging / DMs** | Out **forever** unless explicit product pivot. Spinz is a social-tracking product, not a chat app. The notification surface stays focused. | N/A |
| **Embed/widget API** | "Embed my Spinz diary on my Substack" — niche feature. | Embed-able-content requests show up consistently in feedback |
...
| Feature | Why never |
|---|---|
| **Algorithmic-rec engine that doesn't use social graph** | Spinz's positioning is anti-algorithm; building a generic algo-rec contradicts the wedge |
